...
def _build_alertas_list_payload() -> dict[str, Any]:
    """Construye snapshot_alertas_list agrupado por train_id para el día actual."""
    ts_col = os.getenv("ALERTAS_TIMESTAMP_COL", "receivedDateTime")
    page = _DEFAULT_ALERTAS_LIST_PAGE
    rows = fetch_email_alerts_operational_rows(only_today=True)
    ...
    if snapshot_size == 0:
        return {
            ...
            "links": {
                "self": f"{base_path}?page=1&size=0",
                "next": None,
                "prev": None,
            },
        }
    ...
    return {
        ...
        "links": {
            "self": f"{base_path}?page=1&size={snapshot_size}",
            "next": None,
            "prev": None,
        },
    }
