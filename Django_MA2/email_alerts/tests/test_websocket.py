...
class PollerBroadcastEndToEndTests(TestCase):
    ...
    async def test_poll_broadcasts_to_connected_client(self, mock_fetch_snapshot, mock_fetch_delta):
        ...
        mock_fetch_snapshot.assert_any_call(only_today=True)
        mock_fetch_delta.assert_called_once()
        await communicator.disconnect()

    @patch("email_alerts.tasks.fetch_alertas_since")
    @patch("email_alerts.tasks.fetch_email_alerts_operational_rows")
    async def test_poll_sends_to_train_group(self, mock_fetch_snapshot, mock_fetch_delta):
